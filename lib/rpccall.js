...
module.exports = (connection) => {
    ...
    let client = rpc.Client.$create(connection.port, connection.host, connection.user, connection.pass);
    let caller = {};
    ...
    for(let command in commands){
        ...
        caller[command] = (args, cb) => {
            ...
            client.call(command.toLowerCase(), params, (err, res) => {
                ...
                if (err){
                    return cb(JSON.parse(err.message.substr(5, err.message.length).replace(/\n/g, ' ')));
                }
